...
function _wt_settings_interactive() {
  ...
  while true; do
    result="$(printf '%s' "$(_wt_settings_picker_items)" | fzf --ansi --height=55% --min-height=10 --layout=reverse --padding=1,1 --border=rounded --border-label=' wt settings ' --border-label-pos=2 --header='Enter: toggle/apply  Esc: close' --header-border=rounded --header-label=' keys ' --header-label-pos=2 --info=inline-right --color='border:#888888,label:#d77757,header:#b9c0c8,prompt:#d77757,pointer:#4eba65,info:#b1b9f9,spinner:#d77757,marker:#4eba65,fg:#d9dde3,gutter:-1' --prompt='settings > ' --delimiter=$'\t' --with-nth=3.. --id-nth=1)"
    if [ -z "$result" ]; then
      return 0
    fi
    ...
    case "$item_id" in
      ...
      add-copy)
        ...
        normalized="$(_wt_normalize_copy_path "$new_path")" || continue
        current_paths=("${(@f)$(_wt_get_copy_paths)}")
        if (( ${current_paths[(I)$normalized]} == 0 )); then
      ...
      reset-copy)
        ...
        ;;
      copy:*)
        path_value="${item_id#copy:}"
        _wt_settings copy remove "$path_value" >/dev/null || return 1
        ;;
    esac
  done
}
...
function _wt_settings() {
  ...
  case "$subcommand" in
    ...
    copy)
      ...
      ;;
    *)
      echo "Usage: wt settings <show|edit|env|copy>"
      return 1
      ;;
  esac
}
